...
【心理所想】值
【服装穿着】
【头部】-物品（头部穿戴：发饰、帽子、头巾等，每行一个以"- "开头。若该部位赤裸/无穿戴则写"- 无"）
【上身】-物品（上身穿戴：上衣、外套、披肩等，每行一个以"- "开头。若该部位赤裸则写"- 无"）
【内衣】-物品（内衣穿戴：胸罩、内裤等，每行一个以"- "开头。若未穿则写"- 无"）
【下身】-物品（下身穿戴：裙子、裤子、短裤等，每行一个以"- "开头。若该部位赤裸则写"- 无"）
【腿脚】-物品（腿脚穿戴：袜子、鞋子、腿环等，每行一个以"- "开头。若该部位赤裸则写"- 无"）
【饰品】-物品（配饰：项链、手镯、耳环、戒指等，每行一个以"- "开头。若未佩戴任何饰品则写"- 无"）
每件物品需写出颜色、款式、材质等简要描述，不可只写品类名（如不可只写"袜子"而应写"白色棉质过膝袜"）。
...
_parseOutfit(raw){if(!raw)return[];const items=[];const lines=raw.split('\n');let cz='';for(const line of lines){const t=line.trim();if(!t)continue;const zm=t.match(/^【(.+?)】-?\s*(.*)/);if(zm){const z=zm[1];if(z.includes('头'))cz='head';else if(z.includes('上')||z.includes('身'))cz='top';else if(z.includes('内')||z.includes('胸')||z.includes('裤'))cz='under';else if(z.includes('下')||z.includes('裙'))cz='bottom';else if(z.includes('腿')||z.includes('脚')||z.includes('袜')||z.includes('鞋'))cz='legs';else if(z.includes('饰')||z.includes('手')||z.includes('项'))cz='acc';else cz='';const same=zm[2]?.trim();if(same&&same!=='无'&&same!=='赤裸')items.push({zone:cz,name:same.replace(/（.*?）$/g,'').trim()});continue;}if(cz&&t.startsWith('-')){const n=t.replace(/^-\s*/,'').replace(/（.*?）$/,'').trim();if(n&&n!=='无'&&n!=='赤裸')items.push({zone:cz,name:n});}}return items;}
