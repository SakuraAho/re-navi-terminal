...
_parseOutfit(raw){if(!raw)return[];const items=[];const lines=raw.split('\n');let cz='';for(const line of lines){const t=line.trim();if(!t)continue;const zm=t.match(/^【(.+?)】$/);if(zm){const z=zm[1];if(z.includes('头'))cz='head';else if(z.includes('上')||z.includes('身'))cz='top';else if(z.includes('内')||z.includes('胸')||z.includes('裤'))cz='under';else if(z.includes('下')||z.includes('裙'))cz='bottom';else if(z.includes('腿')||z.includes('脚')||z.includes('袜')||z.includes('鞋'))cz='legs';else if(z.includes('饰')||z.includes('手')||z.includes('项'))cz='acc';continue;}if(cz&&t.startsWith('-'))items.push({zone:cz,name:t.replace(/^-\s*/,'').replace(/（.*?）$/,'').trim()});}return items;}
