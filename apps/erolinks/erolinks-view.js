...
【身体变化】值
【心理所想】值
【服装穿着】⚠️每条"- "独占一行，一行只能写一件衣物。严禁一行内合并多件（如"袜子和鞋子"→必分两行）。每件写颜色+款式+材质。没穿则写"- 无"。

【头部】-物品（帽子一项，若有则独立一行）
【头部-发型】-物品（发型一项，若有则独立一行。若无则写"- 无"）
【头部-发饰】-物品（发饰一项，若有则独立一行。若无则写"- 无"）
【脖子】-物品（围脖、项链、领饰等，每件独立一行。若无则写"- 无"）
【上身躯体-外套】-物品（外套一项，若有则独立一行。若无则写"- 无"）
【上身躯体-内衬】-物品（内衬一项，如衬衫、T恤等，若有则独立一行。若无则写"- 无"）
【上身躯体-胸罩】-物品（胸罩一项，需根据人设推断。若有则独立一行。若无则写"- 无"）
【双手】-物品（手套等，每件独立一行。若无则写"- 无"）
【下身躯体-裤子/裙子】-物品（下身主要衣物一项，裤子或裙子，若有则独立一行。若无则写"- 无"）
【下身躯体-内裤】-物品（内裤一项，需根据人设推断。若有则独立一行。若无则写"- 无"）
【腿脚-袜子】-物品（袜子一项，若有则独立一行。若无则写"- 无"）
【腿脚-鞋子】-物品（鞋子一项，若有则独立一行。若无则写"- 无"）
【装饰】-物品（每个配饰独占一行，如手环、脚环、耳环、戒指、背包各一行。不可将多个配饰写在同一行。若某类配饰有多个（如左手环和右手环），各写一行。若未佩戴任何配饰则写"- 无"）
...
_parseOutfit(raw){if(!raw)return{};const result={head:[],neck:[],upper:[],hands:[],lower:[],legs:[],acc:[]};const lines=raw.split('\n');let cz='';const getZone=z=>{if(z.includes('头')||z.includes('发'))return'head';if(z.includes('脖')||z.includes('领'))return'neck';if(z.includes('上身'))return'upper';if(z.includes('手'))return'hands';if(z.includes('下身'))return'lower';if(z.includes('腿')||z.includes('脚')||z.includes('袜')||z.includes('鞋'))return'legs';if(z.includes('饰')||z.includes('装'))return'acc';return'';};for(const line of lines){const t=line.trim();if(!t)continue;const m=t.match(/^【(.+?)】-?\s*(.*)/);if(m){cz=getZone(m[1]);const same=m[2]?.trim();if(same&&cz)result[cz].push(same.replace(/（.*?）$/g,'').trim());continue;}if(cz&&t.startsWith('-')){const n=t.replace(/^-\s*/,'').replace(/（.*?）$/,'').trim();if(n)result[cz].push(n);}}return result;}
