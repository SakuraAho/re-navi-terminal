...
【身体变化】值
【心理所想】值
【服装穿着】⚠️每条"- "独占一行，一行只能写一件衣物。严禁一行内合并多件（如"袜子和鞋子"→必分两行）。格式："- 短名称 | 详细描述"，短名称≤10字（如"金色双马尾发带"），详细描述写颜色款式材质。若该位置赤裸则写"- 无"。
...
_parseOutfit(raw){if(!raw)return{};const result={head:[],neck:[],upper:[],hands:[],lower:[],legs:[],acc:[]};const lines=raw.split('\n');let cz='';const getZone=z=>{if(z.includes('头')||z.includes('发'))return'head';if(z.includes('脖')||z.includes('领'))return'neck';if(z.includes('上身'))return'upper';if(z.includes('手'))return'hands';if(z.includes('下身'))return'lower';if(z.includes('腿')||z.includes('脚')||z.includes('袜')||z.includes('鞋'))return'legs';if(z.includes('饰')||z.includes('装'))return'acc';return'';};const toItem=n=>{const pipe=n.indexOf('|');const name=pipe>=0?n.substring(0,pipe).trim():n;const desc=pipe>=0?n.substring(pipe+1).trim():n;return{name:name.replace(/（.*?）$/g,'').trim(),desc:desc.replace(/（.*?）$/g,'').trim()};};for(const line of lines){const t=line.trim();if(!t)continue;const m=t.match(/^【(.+?)】-?\s*(.*)/);if(m){cz=getZone(m[1]);const same=m[2]?.trim();if(same&&cz)result[cz].push(toItem(same));continue;}if(cz&&t.startsWith('-')){const n=t.replace(/^-\s*/,'').trim();if(n)result[cz].push(toItem(n));}}return result;}
